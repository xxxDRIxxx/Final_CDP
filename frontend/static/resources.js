...
function renderPosts(posts=[]){
    ...
    return `
      <article class="fb-post reveal">
        <header class="fb-post-head">
          <div class="fb-avatar">
            <img src="/static/uniwiselogo%26text.png" alt="UniWise">
          </div>
          <div class="fb-meta">
  ...
  // Stagger + reveal
  feed.querySelectorAll(".fb-post").forEach((el,i)=>{
...
function bindSpy(){
  ...
  function update(){
    const h = $("resourcesHeader");
    const offset = window.scrollY + (h?h.offsetHeight:0) + 80;
    let cur = null;
    sections.forEach(s=>{
      const el = $(s.id);
      if(el && el.offsetTop <= offset) cur = s;
    });
...
function bindPostStagger(){
  const feed = $("publishedPostsList");
  if(!feed) return;
  const io = new MutationObserver(()=>{
    const posts = feed.querySelectorAll(".fb-post");
    posts.forEach((p,i)=>{
      p.style.transitionDelay = `${i*0.1}s`;
    });
  });
  io.observe(feed, { childList:true });
}
